...
import {
  ...
  PerspectiveCameraHandler,
} from "./app/common/canvas/ICameraHandler";
...
export const DEFAULT_GRID = new GridHelper(
  DEFAULT_GRID_SIZE,
...
export const LESS_FREQUENT_GRID = new GridHelper(
  DEFAULT_GRID_SIZE,
  LESS_FREQUENT_GRID_DIVISIONS,
  0xbbbbbb,
  0xbbbbbb,
);
...
const initialFloorPlanState = (
  ...
  const cameraHandler = new OrthographicCameraHandler(
    new OrthographicCamera(0, 0, 0, 0, 0.1, 500),
    18,
  );
...
const initialInteriorArrangerState = (
  labelRenderer: CSS2DRenderer,
): InteriorArrangerState => {
  const cameraLight = new PointLight(0xffffff, 0.3);
  const cameraHandler = new PerspectiveCameraHandler(
    new PerspectiveCamera(50),
    cameraLight,
    Math.PI,
  );
...
export const App: React.FC = () => {
  ...
  >();
  const [interiorArrangerState, setInteriorArrangerState] = useState<
    InteriorArrangerState | undefined
  >();
  ...
  if (floorPlanState === undefined || interiorArrangerState === undefined) {
    return (
      <div className="center-div-horizontally-and-vertically">
